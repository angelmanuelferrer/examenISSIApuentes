...
import { getAll, remove, sorted } from '../../api/RestaurantEndpoints'
...
export default function RestaurantsScreen ({ navigation, route }) {
        ...
        <Pressable
          ...
          <View style={[{ flex: 1, flexDirection: 'row', justifyContent: 'center' }]}>
            <MaterialCommunityIcons name='sort' color={'white'} size={20}/>
              {item.sortRes === 'order' && <TextRegular textStyle={styles.text}>sort by: order</TextRegular>}
              {item.sortRes === 'price' && <TextRegular textStyle={styles.text}>sort by: price</TextRegular>}
          </View>
        </Pressable>
  ...
  const changeSort = async (restaurant) => {
    try {
      await sorted(restaurant.id)
